...
git_clone() {
 ...
 rm -rf $1
 git clone https://github.com/ezahr/$1.git
 ...
 pwd
 rm -rf $1
 git clone http://gitlab.example.com/root/$1.git
 
 # rsync -av --exclude=".*" src dest 
rsync -av --exclude=".*"  ${GITHUB_PROJECT_DIR}$1 ${GITLAB_PROJECT_DIR}  #maakt em eveentueel aan. 

cd ${GITLAB_PROJECT_DIR}$1
 git add .
 # git status
...
echo "------------"
 ...
 rm -rf $1
 git clone http://192.168.2.11/root/$1.git
# #git clone git@192.168.2.11:root/$1.git
 rsync -av --exclude=".*"  ${GITHUB_PROJECT_DIR}${1}  ${GITLAB_PROJECT_DIR_PI8GB}  #maakt em eveentueel aan. 
 cd ${GITLAB_PROJECT_DIR_PI8GB}$1
 git add .
# # git status
 git commit -m `date +%Y%m%d_%H_%M`
# # git status
